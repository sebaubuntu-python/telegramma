...
async def newpack(update: Update, context: CallbackContext, args: List[str]):
	...
	parser.add_argument("name", help="Short name of sticker set, to be used in t.me/addstickers/ URLs")
	...
	if not update.message.reply_to_message:
		await update.message.reply_text("Reply to a sticker to use as the first sticker in the sticker pack")
		return
...
async def addsticker(update: Update, context: CallbackContext, args: List[str]):
	...
	if not result:
		await update.message.reply_text("Error: It seems that this sticker is already in this sticker pack")
		return
...
async def removesticker(update: Update, context: CallbackContext, args: List[str]):
	if not update.message.reply_to_message:
		await update.message.reply_text("Reply to a sticker message to delete it from a sticker pack")
		return
	...
	if not sticker.set_name:
		await update.message.reply_text("Error: It seems that this sticker isn't in any sticker pack")
		return

	shared_sticker_pack = SharedStickerPack.from_sticker_pack_name(sticker.set_name, context.bot)
	if not shared_sticker_pack:
		await update.message.reply_text(f"Error: I don't know this sticker pack ({sticker.set_name})")
		return
	...
	try:
		...
		await update.message.reply_text(f"Error while deleting sticker from sticker set: {e.message}")
		return

	if not result:
		await update.message.reply_text("Error: It seems that this sticker is not in this sticker pack")
		return
...
async def addadmin(update: Update, context: CallbackContext, args: List[str]):
	...
	if not user_id:
		if not update.message.reply_to_message:
			await update.message.reply_text("Reply to a user message to add them to the admin list of a sticker pack")
			return
		user_id = update.message.reply_to_message.from_user.id

	shared_sticker_pack = SharedStickerPack.from_sticker_pack_name(args.sticker_pack_name, context.bot)
	if not shared_sticker_pack:
		await update.message.reply_text(f"Error: I don't know this sticker pack ({args.sticker_pack_name})")
		return
	...
	await update.message.reply_text(f"User {user_id} added to admin list of sticker pack {shared_sticker_pack.name}")
...
async def removeadmin(update: Update, context: CallbackContext, args: List[str]):
	...
	if not user_id:
		if not update.message.reply_to_message:
			await update.message.reply_text("Reply to a user message to remove them from the admin list of a sticker pack")
			return
		user_id = update.message.reply_to_message.from_user.id
	...
	await update.message.reply_text(f"User {user_id} removed from admin list of sticker pack {shared_sticker_pack.name}")
